...
| Differential testing (vs `llzk-opt`) | ⚠️ Partial | Active when `LLZK_OPT` env or `llzk-opt` on `$PATH`. **3 of 9 tests pass cleanly**: String (`literals`), Include (`from`), Felt verified-pass scaffolding. **6 XFAIL**: Bool (`bool.and` requires `function.def` with `function.allow_non_native_field_ops`), Cast (`cast.toindex` same), RAM (same), Constrain (`function.allow_constraint`), Global (`global.write` requires `function.allow_witness`), Felt arith (LLZK rejects VEIR's `IntegerAttr`-as-`#felt.const` workaround). All XFAILs lift once Function dialect ports (Phase G.1) and/or structured attribute parsing lands. |
